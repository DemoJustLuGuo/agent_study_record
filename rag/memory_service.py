...
class LongTermMemoryService:
    ...
    def add_memory(
        ...
        self.vector_service.vector_store.add_texts(
            chunks, metadatas=[metadata] * len(chunks)
        )
        logger.info(
            f"[memory] stored note for user={user_id}, scope={scope}, project={project}"
        )
        return "【成功】记忆已写入长期存储"

    def search_memory(
        self, query: str, user_id: str = "global", project: str = "", k: int = None
    ) -> str:
        q = (query or "").strip()
        if not q:
            return "【失败】查询内容为空"
