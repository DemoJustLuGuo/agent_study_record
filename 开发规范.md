...
- electron/main.js
	...
	- 支持运行时更新连接参数（backend URL、OPENAI API key），并可按需重启托管后端
- electron/preload.js
	- 通过 contextBridge 暴露受限 API 到渲染层
	- 避免在渲染层直接暴露 ipcRenderer，降低攻击面
	- 暴露连接设置接口（updateConnection / ensureBackendStarted options）供前端配置面板调用
- electron/renderer/*
	- Electron 桌面端 UI
	- 承载聊天、在线 RAG、知识库上传、sync/snapshot/rollback 操作
	- 提供“连接设置”面板，支持填写 API 地址与 API 密钥，并本地持久化（localStorage）
...
- 顶部新增“连接设置”面板：
	- API 地址（如 `http://127.0.0.1:7860`）
	- API 密钥（OpenAI 兼容）
- 点击“保存并连接”后：
	- 配置写入前端 localStorage（仅本机当前桌面端）
	- 调用主进程更新运行时连接参数
	- 按需重启 Electron 托管后端后生效
- 下次打开窗口时：
	- 自动读取本地保存配置，无需重复填写
- “清空设置”会删除本地缓存并回退到默认后端地址
